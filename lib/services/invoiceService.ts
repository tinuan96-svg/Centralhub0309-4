...
class InvoiceService {
  ...
  async getInvoiceById(id: string): Promise<SupplierInvoice | null> {
    const { data, error } = await supabase
      ...
      .select(`
        *,
        suppliers!inner(name, payment_terms, currency)
      `)
      .eq('id', id)
      .maybeSingle();
  ...
  async getSummary(): Promise<InvoiceSummary> {
    ...
    for (const inv of data as any[]) {
      ...
      if (inv.status === 'paid') {
        summary.total_paid += Number(inv.total_amount);
  ...
  async getPODraftsForSupplier(supplierId: string): Promise<{ id: string; total_amount: number; draft_items: any; trigger_reason: string | null }[]> {
    const { data, error } = await supabase
      .from('po_drafts')
      .select('id, total_amount, draft_items, trigger_reason')
