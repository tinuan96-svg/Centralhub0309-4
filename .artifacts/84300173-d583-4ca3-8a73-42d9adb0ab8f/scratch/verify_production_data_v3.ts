...
async function verify() {
  console.log('--- FETCHING PRODUCTION DATA (V3) ---');
  ...
  if (pErr || !allProducts) { console.error('Error fetching products:', pErr); return; }
  ...
  for (const tp of testProducts.slice(0, 10)) {
    const p = tp.product;
  ...
  if (stores && stores.length >= 1 && testProducts.length > 0) {
    console.log('\n--- STORE ISOLATION VERIFICATION ---');
    const store = stores[0];
    const p = testProducts[0].product;

    const backlogStore = orderItems?.filter((oi: any) => oi.product_id === p.id && oi.orders.store_id === store.id)
                        .reduce((sum, o) => sum + (o.quantity || 0), 0) || 0;

    console.log(`Product: ${p.name}`);
    console.log(`  Store ${store.name} Demand: ${backlogStore}`);
    console.log(`  Global Demand (Paid): ${orderItems?.filter(oi => oi.product_id === p.id).reduce((sum, o) => sum + (o.quantity || 0), 0)}`);
  }
}
