-- Align Shruthi live-action command state with the frontend launcher and make
-- voice command inserts/updates observable through Supabase Realtime.

alter table public.voice_assistant_commands
  drop constraint if exists voice_assistant_commands_status_check;

alter table public.voice_assistant_commands
  add constraint voice_assistant_commands_status_check
  check (status = any (array[
    'completed'::text,
    'pending_confirmation'::text,
    'ready_for_computer'::text,
    'failed'::text,
    'cancelled'::text
  ]));
...
begin
  if not exists (
    ...
    where pubname = 'supabase_realtime'
      ...
      and tablename = 'voice_assistant_commands'
  ) then
    alter publication supabase_realtime add table public.voice_assistant_commands;
  end if;
end $$;
