-- Atomically apply a classification head and reconcile the matched bank transactions.
-- The existing classification-only RPC remains unchanged for the "Selected only" action.

CREATE OR REPLACE FUNCTION public.classify_and_reconcile_financial_transactions(
...
BEGIN
  IF NOT public.finance_can_manage() THEN
    RAISE EXCEPTION 'Not authorised to classify and reconcile financial transactions';
  END IF;
...
REVOKE EXECUTE ON FUNCTION public.classify_and_reconcile_financial_transactions(uuid[],text,text,text) FROM anon, PUBLIC;
GRANT EXECUTE ON FUNCTION public.classify_and_reconcile_financial_transactions(uuid[],text,text,text) TO authenticated;
