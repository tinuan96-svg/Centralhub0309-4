...
declare
  ...
  v_system_action boolean := false;
begin
  if lower(coalesce(v_payload->>'browser_required','')) = 'false' then return new; end if;

  v_navigation_verb := v_text ~ '(^|[^a-z])(open|visit|browse|search|look up|lookup|research|check|inspect|go to|navigate|read|click|type|find|scan)([^a-z]|$)';
  v_system_action := v_navigation_verb or v_text ~ '(^|[^a-z])(create|set up|setup|connect|configure|integrate|fix|change|update|manage|add|build|implement|publish|launch|start|work on)([^a-z]|$)';
  ...
  if v_domain is not null then v_domain := regexp_replace(v_domain, '^www\.', ''); end if;
  ...
  if v_url is null then
    if v_text ~ '(merchant center|google merchant|merchant account)' and v_system_action then
    ...
    elsif v_text ~ '(google ads|ads\.google)' and v_system_action then
      v_key := 'google_ads'; v_system := 'Google Ads'; v_url := 'https://ads.google.com/'; v_source := coalesce(v_source,'known_system');
    elsif v_text ~ '(meta business|business manager|facebook business|instagram business)' and v_system_action then
      v_key := 'meta_business'; v_system := 'Meta Business'; v_url := 'https://business.facebook.com/'; v_source := coalesce(v_source,'known_system');
    elsif v_text ~ '(meta developer|developer meta|facebook developer)' and v_system_action then
