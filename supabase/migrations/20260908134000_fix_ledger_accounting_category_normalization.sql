-- Normalize ledger-derived accounting buckets before writing bank_transactions.
-- The bank_transactions constraint uses normalized values such as other and variable_cost.
...
DECLARE
  ...
  v_accounting text := CASE
    WHEN p_transaction_category = 'savings_allocation' THEN 'transfer'
    WHEN p_accounting_category = 'variable_expense' THEN 'variable_cost'
    WHEN p_accounting_category = 'other_income' THEN 'revenue'
    WHEN p_accounting_category IN ('revenue', 'cogs', 'variable_cost', 'operating_expense', 'finance_cost', 'tax', 'asset', 'liability', 'equity', 'transfer', 'other') THEN p_accounting_category
    ELSE 'other'
  END;
