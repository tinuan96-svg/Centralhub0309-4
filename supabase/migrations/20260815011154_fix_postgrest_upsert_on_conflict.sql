/*
  # Fix: Use proper PostgREST upsert with on_conflict parameter
*/
...
BEGIN
  ...
  -- Use merge-duplicates with on_conflict=centralhub_id for upsert
  ...
  IF tg_op = 'DELETE' THEN
    ...
    INSERT INTO public.webhook_logs (
      ...
      'DELETE', coalesce(v_product_id::text, ''), v_product_name,
  ...
  -- Upsert via REST API with on_conflict query param for merge-duplicates
  v_req_id := net.http_post(
