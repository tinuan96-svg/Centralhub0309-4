-- DEFINITIVE FIX: COALESCE types uuid and text cannot be matched
-- Objective: Resolve all remaining type mismatches in trigger functions.

-- 1. Fix products_product_sync_webhook_fn (malluspices sync)
-- Ensuring all UUID variables are cast to text before coalescing with empty strings.
...
BEGIN
  ...
  -- Bypass logic for inactive products
...
-- 2. Fix notify_malluspices_product_webhook
...
-- 3. Fix handle_order_inventory_movement (Ensure all COALESCE calls are type-safe)
