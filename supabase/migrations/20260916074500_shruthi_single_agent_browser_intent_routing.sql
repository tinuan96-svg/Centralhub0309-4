...
create or replace function public.shruthi_complete_source_command_on_session_insert()
returns trigger
language plpgsql
security definer
set search_path to 'pg_catalog','public'
  ...
  if v_command_id is not null then
    update public.voice_assistant_commands
       set status='completed',
           ...
           action_payload=coalesce(action_payload,'{}'::jsonb) || jsonb_build_object('computer_session_id',new.id,'computer_claimed_at',now())
     where id=v_command_id
       and user_id=new.user_id
       and status='ready_for_computer';
  end if;
  return new;
end;
$$;

drop trigger if exists trg_shruthi_complete_source_command_on_session_insert on public.nora_action_sessions;
create trigger trg_shruthi_complete_source_command_on_session_insert
after insert on public.nora_action_sessions
for each row execute function public.shruthi_complete_source_command_on_session_insert();
