...
declare
  ...
  v_setup_verb boolean := false;
  v_web_search_phrase boolean := false;
  v_prev_input text;
...
begin
  ...
  v_setup_verb := v_text ~ '(^|[^a-z])(create|start|set up|setup|connect|configure|integrate|link|register|sign up|signup|enable|add|manage|build)([^a-z]|$)' or v_text ~ '(സെറ്റപ്പ്|ക്രിയേറ്റ്|കണക്റ്റ്|തുടങ്ങ|ചെയ്യ|செட்டப்|கிரியேட்|கனெக்ட்|தொடங்கு|செய்)';
  v_web_search_phrase := v_text ~ '(search (the )?web|search on (the )?web|google search|look up online|lookup online|find online|research online|web search)';
  ...
  v_store_action := (v_navigation_verb or v_setup_verb) and v_text ~ '(mallu\s*spices|malluspices|kerala\s*grocery|keralagrocery|pocket\s*grocery|pocketgrocery|tamil\s*retail|tamilretail)';
  ...
  if v_url is null then
    if v_text ~ '(merchant center|google merchant|merchant account)' and (v_navigation_verb or v_setup_verb) then
    ...
    elsif v_text ~ '(google ads|ads\.google)' and (v_navigation_verb or v_setup_verb) then
      v_key := 'google_ads'; v_system := 'Google Ads'; v_url := 'https://ads.google.com/'; v_source := coalesce(v_source,'known_system');
    elsif v_text ~ '(meta business|business manager|business suite|facebook business|instagram business|meta ads|facebook ads|ads manager)' and (v_navigation_verb or v_setup_verb) then
      v_key := 'meta_business'; v_system := 'Meta Business'; v_url := 'https://business.facebook.com/'; v_source := coalesce(v_source,'known_system');
    elsif v_text ~ '(meta developer|meta for developers|developer meta|facebook developer|developer app)' and (v_navigation_verb or v_setup_verb) then
    ...
    elsif v_text ~ 'instagram' and v_setup_verb then
      v_key := 'instagram'; v_system := 'Instagram'; v_url := 'https://www.instagram.com/'; v_source := coalesce(v_source,'known_system');
    elsif v_text ~ 'facebook' and v_setup_verb then
      v_key := 'facebook'; v_system := 'Facebook'; v_url := 'https://www.facebook.com/'; v_source := coalesce(v_source,'known_system');
    elsif v_text ~ 'supabase' and (v_navigation_verb or v_setup_verb) then
      v_key := 'supabase'; v_system := 'Supabase'; v_url := 'https://supabase.com/dashboard'; v_source := coalesce(v_source,'known_system');
    elsif v_text ~ 'netlify' and (v_navigation_verb or v_setup_verb) then
      v_key := 'netlify'; v_system := 'Netlify'; v_url := 'https://app.netlify.com/'; v_source := coalesce(v_source,'known_system');
    elsif v_text ~ 'github' and (v_navigation_verb or v_setup_verb) then
    ...
    elsif v_web_search_phrase then
      v_key := 'web_search'; v_system := 'Web Search'; v_url := 'https://www.google.com/'; v_source := coalesce(v_source,'explicit_web_search');
