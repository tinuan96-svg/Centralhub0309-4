...
Deno.serve(async req => {
  ...
  try {
    const body = await req.json().catch(() => ({})), providerId = String(body?.providerId || 'google').trim(), action = String(body?.action || 'get')
    const db = await requireAdmin(req)

    if (providerId === 'meta') {
      return json({
        success: false,
        error: 'Meta developer apps are store-specific. Select a store under Marketing > Integrations and configure that store Meta App ID and App Secret there.',
        provider_id: 'meta',
        scope_mode: 'store_owned',
        required_redirect_uri: callback('meta'),
      }, 409)
    }
    ...
    return json({ success: false, error: message, provider_id: 'google', required_redirect_uri: callback('google') }, /Authorization|session|Admin access/i.test(message) ? 401 : 400)
  }
})