...
async function key() { const material = env('MARKETING_TOKEN_ENCRYPTION_KEY'); let raw: Uint8Array; try { raw = unb64(material) } catch { raw = new Uint8Array() }; if (raw.length !== 32) raw = new Uint8Array(await crypto.subtle.digest('SHA-256', new TextEncoder().encode(material))); return crypto.subtle.importKey('raw', raw, 'AES-GCM', false, ['encrypt']) }
