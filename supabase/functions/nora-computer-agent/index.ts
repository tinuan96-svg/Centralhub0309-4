...
Deno.serve(async (req) => {
  ...
  if (action === "start") {
    ...
    if (!state.pageReady) return json(409, { success: false, error: "page_not_ready" });
    ...
    openaiBody = {
      ...
      input: [
        { type: "computer_call_output", call_id: previousCallId, output: { type: "computer_screenshot", image_url: `data:image/png;base64,${screenshot}`, detail: "original" } },
