...
async function findNetlifySite(token: string, domain: string) {
  ...
  if (!r.ok) throw new Error(`netlify_sites_${r.status}`);
  const sites = await r.json().catch(() => []) as Array<Record<string, unknown>>;
  const wanted = cleanHost(domain);
...
async function getNetlifySite(token: string, siteId: string | null, domain: string) {
  if (siteId) {
    const r = await fetch(`https://api.netlify.com/api/v1/sites/${encodeURIComponent(siteId)}`, { headers: { Authorization: `Bearer ${token}` } });
    if (r.ok) return await r.json().catch(() => null) as Record<string, any> | null;
    if (r.status !== 404) throw new Error(`netlify_site_${r.status}`);
  }
  return await findNetlifySite(token, domain) as Record<string, any> | null;
}

async function triggerNetlifyBuild(token: string, siteId: string) {
  const r = await fetch(`https://api.netlify.com/api/v1/sites/${encodeURIComponent(siteId)}/builds`, {
    method: "POST",
...
async function annotateBlocked(db: ReturnType<typeof createClient>, deployment: Record<string, any>, reason: string, extra: Record<string, unknown> = {}) {
  await db.from("site_health_deployments").update({
    details: {
      ...(deployment.details ?? {}),
      verifier: "site-health-deploy-verifier",
      verifier_blocked: reason,
      verifier_checked_at: new Date().toISOString(),
      ...extra,
    },
  }).eq("id", deployment.id);
  return { id: deployment.id, status: "pending", reason, ...extra };
}

async function markBuilding(db: ReturnType<typeof createClient>, deployment: Record<string, any>, details: Record<string, unknown>) {
  await db.from("site_health_deployments").update({
    status: "building",
    details: {
      ...(deployment.details ?? {}),
  ...
  return { id: deployment.id, status: "building", ...details };
}
...
async function verifyOne(db: ReturnType<typeof createClient>, deployment: Record<string, any>, netlifyToken: string) {
  ...
  const { data: issue } = await db.from("site_health_issues")
    .select("id,store_id,status,check_name")
    .eq("id", attempt.issue_id)
    .maybeSingle();
  if (!issue?.store_id) return annotateBlocked(db, deployment, "issue_missing");

  const { data: config } = await db.from("site_health_store_configs")
    .select("store_id,domain,github_repo,production_branch,deploy_provider,master_enabled,kill_switch,verify_after_deploy,netlify_site_id")
    .eq("store_id", issue.store_id)
    .maybeSingle();
  if (!config || !config.master_enabled || config.kill_switch || !config.verify_after_deploy) return annotateBlocked(db, deployment, "verification_disabled");
  if ((config.deploy_provider ?? "netlify") !== "netlify") return annotateBlocked(db, deployment, "unsupported_provider");
  if (!netlifyToken) return annotateBlocked(db, deployment, "netlify_auth_token_missing");

  const site = await getNetlifySite(netlifyToken, config.netlify_site_id ? String(config.netlify_site_id) : null, String(config.domain ?? ""));
  if (!site?.id) return annotateBlocked(db, deployment, "netlify_site_not_found");

  const deploysResp = await fetch(`https://api.netlify.com/api/v1/sites/${encodeURIComponent(String(site.id))}/deploys?per_page=100`, { headers: { Authorization: `Bearer ${netlifyToken}` } });
  if (!deploysResp.ok) throw new Error(`netlify_deploys_${deploysResp.status}`);
  const deploys = await deploysResp.json().catch(() => []) as Array<Record<string, any>>;
  ...
  if (!matched) {
    const exactFailed = deploys.find((d) => String(d.commit_ref ?? d.commit_sha ?? "") === String(attempt.commit_sha) && ["error", "failed"].includes(String(d.state ?? "")));
    if (exactFailed) {
      await db.from("site_health_deployments").update({
        deploy_id: exactFailed.id == null ? null : String(exactFailed.id),
        deploy_url: exactFailed.ssl_url ?? exactFailed.deploy_ssl_url ?? exactFailed.url ?? null,
        status: "failed",
        production_verified: false,
        details: { ...(deployment.details ?? {}), verifier: "netlify_api", netlify_state: exactFailed.state, verified_reason: "deploy_failed" },
      }).eq("id", deployment.id);
      await db.from("site_health_verifications").insert({ issue_id: issue.id, deployment_id: deployment.id, source: "netlify", status: "fail", details: { reason: "deploy_failed", netlify_deploy_id: exactFailed.id ?? null } });
      await db.from("site_health_issues").update({ status: "failed" }).eq("id", issue.id);
      return { id: deployment.id, status: "failed", reason: "deploy_failed" };
    }
    ...
    return markBuilding(db, deployment, {
  ...
  const deployUrl = matched.ssl_url ?? matched.deploy_ssl_url ?? matched.url ?? null;
  await db.from("site_health_deployments").update({
    deploy_id: matched.id == null ? null : String(matched.id),
  ...
  await db.from("site_health_verifications").insert({
    issue_id: issue.id,
...
Deno.serve(async (req: Request) => {
  ...
  if (!url || !serviceRole) return reply(503, { ok: false, error: "supabase_not_configured" });
  ...
  const { data: pending, error } = await db.from("site_health_deployments")
    .select("id,fix_attempt_id,provider,commit_sha,status,details,created_at")
    .in("status", ["pending", "building", "deployed"])
    .eq("production_verified", false)
    .order("created_at", { ascending: true })
    .limit(10);
  if (error) return reply(500, { ok: false, error: "pending_read_failed" });
