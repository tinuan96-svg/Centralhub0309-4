...
async function getSnapshot(db: any, userId: string, userText: string) {
  ...
  if (wants("competitors")) extras.push({ key: "competitors", query: db.from("competitor_prices").select("competitor_id,product_id,price,currency,scan_status,scan_error,match_status,data_quality_state,last_scanned_at,updated_at").order("updated_at", { ascending: false }).limit(50) });
