...
Deno.serve(async (req: Request) => {
  ...
  if (action === "command") {
    ...
    for (const model of candidateModels) {
      ...
      aiResponse = await fetch("https://api.openai.com/v1/responses", {
        ...
        body: JSON.stringify({
          model,
          store: false,
          reasoning: { effort: deepRequest ? "medium" : "none" },
          input: prompt,
          max_output_tokens: deepRequest ? 1200 : 450,
          text: {
            format: {
              type: "json_schema",
    ...
    const finalResult = {
      reply: String(result?.reply ?? "Shruthi is ready.").slice(0, 5000),
