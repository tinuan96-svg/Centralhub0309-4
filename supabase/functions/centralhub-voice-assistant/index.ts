...
Deno.serve(async (req: Request) => {
  ...
  if (action === "transcribe") {
    ...
    try {
      ...
      form.append("prompt", "NORA is the CentralHub AI executive assistant. The speaker may code-switch between Malayalam and English. Important terms include NORA, CentralHub, MalluSpices, KeralaGrocery, PocketGrocery, Supabase, GitHub, Netlify, DHL, Mollie, WhatsApp, dashboard, orders, profit, stock, competitors, finance, security, board meeting, scan, deploy, notifications.");
  ...
  if (action === "command") {
    ...
    const finalResult = {
      reply: String(result?.reply ?? "NORA is ready.").slice(0, 5000),
    ...
    const { error: historyError } = await db.from("voice_assistant_commands").insert({
      ...
      action_payload: { navigation_path: finalResult.navigation_path, model: usedModel, page_context: pageContext, assistant_name: "NORA" },
      status,
    });
