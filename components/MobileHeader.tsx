...
export default function MobileHeader() {
  ...
  useEffect(() => {
    setMounted(true);
    // REMOVED: Global store selector logic. Stores no longer needed here.
    checkAuth();
  ...
  const allNavItems = useMemo(() => [
    ...
    },
    {
      ...
      subItems: [
        ...
        { href: '/marketing/analytics', label: 'Marketing Analytics' },
        ...
        <div className="fold-inner:hidden fixed inset-0 z-[60] flex flex-col bg-slate-950 animate-in fade-in duration-200">
          <div className="flex items-center justify-between px-4 h-14 border-b border-slate-800">
            <div className="flex items-center gap-3">
              ...
              <h2 className="text-lg font-black text-white uppercase tracking-tighter">CentralHub</h2>
            </div>
            <button
          ...
          <div className="p-4 border-b border-slate-900 bg-slate-950">
            <div className="relative">
              <input
                ...
                className="w-full bg-slate-900 border border-slate-800 rounded-xl px-4 py-2.5 text-sm text-white placeholder-slate-500 focus:outline-none focus:ring-2 focus:ring-blue-500/50 transition-all"
              />
              {navSearch && (
                <button
                    ...
                    <>
                      <button
                        ...
                        className={`w-full flex items-center justify-between px-4 py-3 rounded-xl transition-all ${
                          isActive ? 'bg-blue-500/10 text-blue-400 border border-blue-500/20' : 'text-slate-400 hover:bg-slate-900'
                        }`}
                      >
                        <div className="flex items-center gap-3">
                          ...
                          {item.subItems.map((sub) => (
                            <Link
                              ...
                              className={`block px-4 py-2.5 rounded-lg text-sm transition-colors ${
                                pathname === sub.href ? 'text-blue-400 font-bold' : 'text-slate-500 hover:text-slate-300'
                              }`}
                            >
                              {sub.label}
                            </Link>
          ...
          <div className="p-4 border-t border-slate-800 bg-slate-900/50 pb-safe-bottom">
            <div className="flex items-center gap-3 px-4 py-3 rounded-xl bg-slate-900 border border-slate-800 mb-4">
              <div className="w-10 h-10 rounded-full bg-gradient-to-r from-blue-500 to-purple-600 flex items-center justify-center font-black text-white">
        ...
        <div className="fold-inner:hidden fixed inset-0 bg-black/80 backdrop-blur-sm z-50 flex items-end animate-in fade-in duration-300" onClick={() => setShowUserMenu(false)}>
          <div className="w-full bg-slate-900 rounded-t-[2rem] border-t border-slate-800 animate-in slide-in-from-bottom duration-300" onClick={(e) => e.stopPropagation()}>
            <div className="px-6 py-5 border-b border-slate-800 flex items-center justify-between">
              <h3 className="text-lg font-black text-white uppercase tracking-tight">Account</h3>
              <button
            ...
            <div className="p-6 space-y-6 pb-safe-bottom">
              <div className="flex items-center gap-4 p-4 rounded-2xl bg-slate-800/50 border border-slate-700/30">
                <div className="w-14 h-14 rounded-full bg-gradient-to-br from-blue-500 to-purple-600 flex items-center justify-center shadow-xl ring-2 ring-blue-500/20">
                ...
                <div className="min-w-0">
                  ...
                  <p className="text-xs font-black text-slate-500 uppercase tracking-widest">Administrator</p>
                </div>
              </div>
              ...
              <div className="grid grid-cols-1 gap-3">
                <button
                  onClick={handleLogout}
                  className="w-full flex items-center justify-center gap-2 px-4 py-4 bg-rose-900/20 text-rose-500 border border-rose-500/20 font-black uppercase tracking-widest rounded-2xl hover:bg-rose-900/30 transition-all active:scale-95"
                >
                  <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M17 16l4-4m0 0l-4-4m4 4H7m6 4v1a3 3 0 01-3 3H6a3 3 0 01-3-3V7a3 3 0 013-3h4a3 3 0 013 3v1" />
                  </svg>
