...
import { Fingerprint, KeyRound, LockKeyhole, Mic, ShieldCheck, X } from 'lucide-react';
...
type SecureUnlockEvent = CustomEvent<{ success?: boolean; reason?: string }>;
...
export default function ShruthiSecurityGate() {
  ...
  useEffect(() => {
    const onTranscript = (event: Event) => {
      ...
      // Global conversational stop guard. This executes before the assistant's
      // normal transcript listener so phrases such as “okay stop” never become
      // another AI request and never produce a spoken acknowledgement loop.
    ...
    const onSecureUnlock = (event: Event) => {
      const detail = (event as SecureUnlockEvent).detail || {};
      if (detail.success) {
        unlock();
      } else {
        setStatus('Identity verification not completed');
        setError(detail.reason === 'cancelled' ? 'Verification cancelled. Try again or use secure login.' : 'Device identity did not verify. Try again or use secure login.');
        setFallback(true);
      }
    };

    window.addEventListener('centralhub:tara-transcript', onTranscript as EventListener, true);
    window.addEventListener('centralhub:secure-unlock', onSecureUnlock as EventListener);
    return () => {
      window.removeEventListener('centralhub:tara-transcript', onTranscript as EventListener, true);
      window.removeEventListener('centralhub:secure-unlock', onSecureUnlock as EventListener);
    };
  }, [unlock]);

  const useDeviceVerification = () => {
    setError('');
    setStatus('Waiting for secure device verification…');
    let started = false;
    try { started = nativeBridge()?.requestSecureUnlock?.() === true; } catch { started = false; }
  ...
  return (
    <section className="fixed inset-0 z-[320] overflow-y-auto bg-[#01040a] text-white" aria-label="Shruthi Security Login">
      ...
      <div className="relative mx-auto flex min-h-[100dvh] w-full max-w-3xl flex-col items-center justify-center px-5 py-[max(28px,env(safe-area-inset-top))] text-center">
        <div className="mb-7 flex items-center gap-2 text-sm font-semibold tracking-[0.28em] text-slate-400">
          <ShieldCheck className="h-4 w-4 text-cyan-300" /> CENTRALHUB SECURE ACCESS
        </div>
        ...
        <div className="mt-8 w-full max-w-xl rounded-3xl border border-cyan-300/15 bg-slate-950/70 p-5 shadow-2xl backdrop-blur-xl">
          ...
          <p className="mt-4 text-[11px] leading-relaxed text-slate-500">The spoken phrase starts the security flow. Android biometric/device credential performs the actual identity verification; Shruthi does not pretend speech-to-text alone is a secure voiceprint.</p>
          {error && <p className="mt-3 text-sm text-rose-300">{error}</p>}

          <div className="mt-5 grid gap-2 sm:grid-cols-2">
            <button type="button" onClick={useDeviceVerification} className="inline-flex items-center justify-center gap-2 rounded-2xl border border-cyan-300/20 bg-cyan-300/10 px-4 py-3 text-sm font-semibold text-cyan-100">
              <Fingerprint className="h-4 w-4" /> Verify securely
            </button>
            <button type="button" onClick={() => setFallback((value) => !value)} className="inline-flex items-center justify-center gap-2 rounded-2xl border border-white/10 bg-white/[0.035] px-4 py-3 text-sm font-semibold text-slate-200">
              <KeyRound className="h-4 w-4" /> Login ID & password
            </button>
          </div>
