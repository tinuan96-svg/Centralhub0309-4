...
interface TimeSeriesChartProps { series: ChartSeries[]; timeRange: string; title?: string; unit?: 'GBP' | 'count'; subtitle?: string; compact?: boolean; }

export default function TimeSeriesChart({ series, timeRange, title = 'Revenue per store', unit = 'GBP', subtitle, compact = false }: TimeSeriesChartProps) {
  ...
  return <Panel className={compact ? 'ch-chart-compact' : ''} title={title} subtitle={subtitle} action={<span className="ch-status">{unit === 'GBP' ? 'GBP' : 'Count'} · {timeRange === 'custom' ? 'Selected dates' : timeRange}</span>}>
    ...
    {!dates.length ? <EmptyState>No trend data for this selection.</EmptyState> : <>
      <div ref={chartRef} className="ch-chart-scroll" tabIndex={0} role="region" aria-label={title + ' chart. Scroll horizontally on small screens.'}><svg viewBox={`0 0 ${width} 258`} className="ch-chart" role="img" aria-label={title + '. ' + dates.length + ' dates. Exact values are in the table below.'} onPointerMove={e => { const rect = e.currentTarget.getBoundingClientRect(); const px = (e.clientX - rect.left) / rect.width * width; const i = dates.length === 1 ? 0 : Math.max(0, Math.min(dates.length - 1, Math.round((px - 66) / plotWidth * (dates.length - 1)))); setSelected(dates[i]); }}>
