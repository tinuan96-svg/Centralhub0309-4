...
interface TimeSeriesChartProps { series: ChartSeries[]; timeRange: string; title?: string; unit?: 'GBP' | 'count'; subtitle?: string; }

export default function TimeSeriesChart({ series, timeRange, title = 'Revenue per store', unit = 'GBP', subtitle }: TimeSeriesChartProps) {
  ...
  const compact = (n: number) => new Intl.NumberFormat('en-GB', { notation: 'compact', maximumFractionDigits: 1, ...(unit === 'GBP' ? { style: 'currency', currency: 'GBP' } : {}) }).format(n);
  ...
  const x = (i: number) => dates.length === 1 ? 348 : 66 + i / (dates.length - 1) * 560;
  const y = (v: number) => 220 - (v - low) / range * 185;
  const valueAt = (s: ChartSeries, d: string) => s.data.find(p => p.date === d)?.value;
  const step = Math.max(1, Math.ceil(dates.length / 5));
  return <Panel title={title} subtitle={subtitle} action={<span className="ch-status">{unit === 'GBP' ? 'GBP' : 'Count'} · {timeRange === 'custom' ? 'Selected dates' : timeRange}</span>}>
    ...
    {!dates.length ? <EmptyState>No trend data for this selection.</EmptyState> : <>
      <div className="ch-chart-scroll" tabIndex={0} role="region" aria-label={title + ' chart. Scroll horizontally on small screens.'}><svg viewBox="0 0 660 258" className="ch-chart" role="img" aria-label={title + '. ' + dates.length + ' dates. Exact values are in the table below.'} onPointerMove={e => { const rect = e.currentTarget.getBoundingClientRect(); const px = (e.clientX - rect.left) / rect.width * 660; const i = dates.length === 1 ? 0 : Math.max(0, Math.min(dates.length - 1, Math.round((px - 66) / 560 * (dates.length - 1)))); setSelected(dates[i]); }}>
        ...
        {Array.from({ length: 5 }, (_, i) => { const val = low + i / 4 * range; return <g key={i}><line x1="66" x2="626" y1={y(val)} y2={y(val)} stroke="#303956" strokeDasharray="3 5" /><text x="55" y={y(val) + 4} textAnchor="end" fill="#a0acc7" fontSize="12">{compact(val)}</text></g>; })}
        ...
        <line x1={x(index)} x2={x(index)} y1="25" y2="225" stroke="#c4b5fd" strokeOpacity=".35" strokeDasharray="4 4" />
        {dates.map((d, i) => (i % step === 0 || i === dates.length - 1) && <text key={d} x={x(i)} y="250" textAnchor="middle" fill="#a0acc7" fontSize="12">{dateLabel(d)}</text>)}
      </svg></div>
