...
const requiredFiles = [
  ...
  'app/banking/BankingClient.tsx',
  'app/business-intelligence/page.tsx',
  'app/business-intelligence/automation/page.tsx',
  'app/business-intelligence/automation/AutomationControlCentre.tsx',
  'app/competitors/page.tsx',
  'app/customer-care/page.tsx',
  'app/dashboard/page.tsx',
  'app/finance/FinancialCommandSummary.tsx',
  'app/finance/vat/page.tsx',
  'app/finance/planning/page.tsx',
  'app/finance/planning/FinancialPlanningClient.tsx',
  'supabase/functions/financial-planning-ai/index.ts',
  'supabase/migrations/20260908114206_financial_planning_growth_centre.sql',
  'app/finance/mollie/page.tsx',
  'app/finance/mollie/MollieAuditClient.tsx',
  'supabase/functions/mollie-audit-ingest/index.ts',
  'supabase/migrations/20260908120000_restore_mollie_audit_visibility.sql',
  ...
  'lib/services/banking/bankSyncService.ts',
  'lib/services/suppliers/supplierService.ts',
  ...
  'supabase/functions/customer-care-ai/index.ts',
  ...
  'supabase/migrations/20260906222700_restore_expiry_loss_history.sql',
  'supabase/migrations/20260906224400_seed_automation_competitor_setting.sql',
];
...
const assertions = [
  ...
  { label: 'Security monitor performs real external probes and records honest states', file: 'supabase/functions/security-monitor/index.ts', all: ['AbortSignal.timeout', "let status: 'online' | 'degraded' | 'offline' = 'offline'", "from('security_heartbeats')", "from('security_events')"] },
  ...
  { label: 'Customer-care webhook keeps AI-processing state and escalation push handoff', file: 'supabase/functions/whatsapp-webhook/index.ts', all: ['ai_processing_started_at', 'sendExistingCentralHubNotification', 'AI_ESCALATION:'] },
  { label: 'Customer-care AI escalations remain immediately visible to humans', file: 'supabase/functions/customer-care-ai/index.ts', all: ['sendExistingCentralHubNotification', 'AI_ESCALATION:${metadata.ticket_id}', 'system_notifications'] },
  { label: 'Customer-message watchdog preserves the prompt 45-second unanswered boundary and idempotency key', file: 'netlify/functions/customer-message-watchdog.ts', all: ['UNANSWERED_AFTER_MS = 45 * 1000', 'CUSTOMER_MESSAGE_RECEIVED_OR_UNANSWERED', 'CUSTOMER_MESSAGE:'] },
  ...
  { label: 'Expiry loss ledger migration permanently captures expired stock reductions', file: 'supabase/migrations/20260906222700_restore_expiry_loss_history.sql', all: ['inventory_expiry_writeoffs', 'capture_expired_inventory_writeoff', 'trg_capture_expired_inventory_writeoff', 'expired_stock_reduction'] },
  { label: 'Supplier service blocks equivalent active supplier duplicates before and at the database boundary', file: 'lib/services/suppliers/supplierService.ts', all: ['normalizeSupplierName', 'findEquivalentActiveSupplier', "error.code === '23505'", 'Enterprise/Enterprises'] },
  { label: 'Banking UI and dashboard share the canonical store bank account source', file: 'lib/services/banking/bankSyncService.ts', all: ["from('store_bank_accounts')", 'current_balance', 'getBankAccounts'] },
  { label: 'Dashboard finance summary reads canonical financial performance including bank closing cash', file: 'app/finance/FinancialCommandSummary.tsx', all: ["rpc('get_financial_performance'", 'closing_cash', 'Bank Closing'] },
  { label: 'Mollie audit page remains visible and reads the central audit mirror', file: 'app/finance/mollie/MollieAuditClient.tsx', all: ['from("mollie_webhook_events")', 'Mollie Audit Centre', 'mollie_balance_transactions'] },
  { label: 'Mollie audit ingest remains service-secret protected and store-scoped', file: 'supabase/functions/mollie-audit-ingest/index.ts', all: ['SUPABASE_SERVICE_ROLE_KEY', 'CENTRALHUB_PUSH_API_SECRET', 'malluspices', 'mollie_webhook_events'] },
  { label: 'Mollie audit table remains admin-readable and service-writable', file: 'supabase/migrations/20260908120000_restore_mollie_audit_visibility.sql', all: ['enable row level security', 'public.is_admin()', 'grant select', 'mollie_webhook_events'] },
  { label: 'Automation route stays inside the authenticated app shell and renders the control centre', file: 'app/business-intelligence/automation/page.tsx', all: ['AutomationControlCentre'] },
  { label: 'Automation Control Centre persists settings/logs and preserves kill switch, DRY RUN and explicit confirmation before live execution', file: 'app/business-intelligence/automation/AutomationControlCentre.tsx', all: ['system_intelligence_settings', 'automation_execution_log', 'automation_global_enabled', 'automation_mode_active', 'automation_competitor_enabled', '.upsert(', 'window.confirm', 'DRY RUN', 'STOP ALL'] },
  { label: 'Automation competitor safety setting is seeded without turning it live', file: 'supabase/migrations/20260906224400_seed_automation_competitor_setting.sql', all: ['automation_competitor_enabled', 'false', 'on conflict'] },
