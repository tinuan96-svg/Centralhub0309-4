...
for (const tableName in schema) {
  ...
  // Relationships
  ts += `        Relationships: [\n`;
  // Add common relationships based on column naming conventions
  for (const colName in table.columns) {
      if (colName.endsWith('_id') && colName !== 'id') {
          const targetTable = colName.slice(0, -3) + 's';
          // Check if target table exists in schema
          if (schema[targetTable] || schema[colName.slice(0,-3)]) {
               const actualTarget = schema[targetTable] ? targetTable : colName.slice(0,-3);
               ts += `          {\n`;
               ts += `            foreignKeyName: "${tableName}_${colName}_fkey",\n`;
               ts += `            columns: ["${colName}"],\n`;
               ts += `            referencedRelation: "${actualTarget}",\n`;
               ts += `            referencedColumns: ["id"],\n`;
               ts += `          },\n`;
          }
      }
  }
...
console.log('lib/supabase.ts updated v4');
