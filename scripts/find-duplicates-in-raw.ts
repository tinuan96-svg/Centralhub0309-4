...
Nirapara	Rasam Powder	rasam-powder	Pickles & Preserves	2.16	1.49	12	400	g	FALSE	TRUE	MS-000146
...
Homely	Pork Masala	pork-masala-165-gm-by-homely-9372	Curry Masalas	3.45	1.88	5	0.165	g	FALSE	TRUE	PMâ€“H-165	30/06/2027
...
function findDuplicates() {
    ...
    rows.forEach(row => {
        ...
        const weight = cols[7]?.trim();
        const unit = cols[8]?.trim();

        // Simulate script's cleaning logic
        let cleanName = name
        ...
        const key = `${brand}|${cleanName}|${category}`.toLowerCase();

        let w = parseFloat(weight) || null;
        let u = unit || 'Kg';
        if (w === null) {
            const match = name.match(/(\d+(\.\d+)?)\s*(kg|g|l|ml|pcs|pods|bags)/i);
            if (match) {
                w = parseFloat(match[1]);
                u = match[3].toLowerCase();
            }
        }
        const attribute = w ? `${w} ${u}` : null;

        const list = groups.get(key) || [];
        list.push({ brand, name, cleanName, category, weight, unit, attribute });
        groups.set(key, list);
    });

    console.log("Products with duplicate variants (same attribute):");
    groups.forEach((items, key) => {
        if (items.length > 1) {
            const attributes = items.map(i => i.attribute);
            const duplicates = attributes.filter((attr, index) => attributes.indexOf(attr) !== index);
            if (duplicates.length > 0) {
                console.log(`\nKey: ${key}`);
                items.forEach(i => {
                    console.log(`  - Original: "${i.name}" | Attribute: "${i.attribute}"`);
                });
            }
        }
    });
