...
import { isPaidOrder, numeric, productRevenue, productSubtotal } from '@/lib/dashboard/metrics';
...
export default function ReferenceDashboard({ report, selectedStoreId, timeRange }: { report: DashboardReport; selectedStoreId: string; timeRange: string }) {
  ...
  for (const row of report.current.costRows) { const date = orderDays.get(row.id); if (date) profitByDay.set(date, (profitByDay.get(date) || 0) + row.gross_profit); }
  const momentum = [{ id: 'sales', name: 'Paid order total', color: '#39e6ef', data: dates.map(date => ({ date, value: calendar.get(date)?.revenue || 0 })) }, ...(report.current.actualGrossProfit === null ? [] : [{ id: 'gross', name: 'Order gross profit', color: '#93c5fd', data: dates.map(date => ({ date, value: profitByDay.get(date) || 0 })) }])];
  ...
  for (const order of paid) { const method = String(order.payment_method || 'Unknown').replace(/_/g, ' '); methodMap.set(method, (methodMap.get(method) || 0) + 1); }
  const paymentGroups = [...methodMap.entries()].sort((a, b) => b[1] - a[1]).slice(0, 4).map(([label, value], i) => ({ label, value, color: MODEL_COLORS[i % MODEL_COLORS.length] }));
  ...
  return <div className="ch-model-grid">
    ...
    <Panel title="Paid payment methods" subtitle="Payment methods · paid orders only" className="ch-model-positions">
      <div className="ch-model-dot-rows">{paymentGroups.map(row => { const percent = ratio(row.value, paid.length); return <div key={row.label}><span>{row.label}</span><div className="ch-model-dots" aria-hidden="true">{Array.from({ length: 10 }, (_, i) => <i key={i} style={{ background: percent !== null && i < Math.round(percent / 10) ? row.color : 'var(--model-track)' }} />)}</div><strong>{percent === null ? '—' : percent.toFixed(1) + '%'}</strong><small>{row.value}</small></div>; })}{paymentGroups.length === 0 && <p className="text-xs text-slate-500">No paid orders in this period.</p>}</div>
    </Panel>
    <section className="ch-panel ch-model-trend-profile" aria-label="Sales trend and operating ratios">
      <div className="ch-model-trend"><TimeSeriesChart compact dense series={momentum} timeRange={timeRange} title="Paid order total & gross profit" subtitle={report.current.actualGrossProfit === null ? 'GBP · Profit unavailable: incomplete costs' : 'GBP · Order totals include delivery charged'} /></div>
      <HealthRadar axes={[{ label: 'Buyer ID', value: ratio(identifiedOrders, paid.length), detail: 'Paid orders with identified buyer email' }, { label: 'Delivery', value: ratio(delivered, paid.length), detail: 'Delivered / paid orders' }, { label: 'Repeat', value: ratio(repeat, buyers.size), detail: 'Repeat / identified buyers' }, { label: 'Costs', value: ratio(recorded, paid.length), detail: 'Recorded costs / paid orders' }, { label: 'Stock', value: ratio(stocked, report.inventory.length), detail: 'Positive stock / stock records' }]} />
    </section>
    ...
    <Panel title="Store sales mix" subtitle="Paid order totals · selected period" className="ch-store-mix">
      {scatter.map((store, index) => { const share = ratio(store.revenue, report.current.totalRevenue); return <div className="ch-store-mix-row" key={store.id}><div><span>{store.name}</span><strong>{formatCurrency(store.revenue)}</strong></div><div className="ch-rate-track" aria-hidden="true"><i style={{ width: Math.max(0, Math.min(100, share ?? 0)) + '%', background: MODEL_COLORS[index % MODEL_COLORS.length] }} /></div><small>{share === null ? 'No sales baseline' : share.toFixed(1) + '% of paid totals'} · {paid.filter(order => (order.store_id || 'unassigned') === store.id).length} paid orders</small></div>; })}
    </Panel>
