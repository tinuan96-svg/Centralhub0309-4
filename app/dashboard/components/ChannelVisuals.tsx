...
export default function ChannelVisuals({ start, end, selectedStoreId, refreshKey }: { start: string; end: string; selectedStoreId: string; refreshKey: number }) {
  ...
  useEffect(() => {
    let cancelled = false;
    setAnalytics(state => ({ ...state, loading: true })); setMarketing(state => ({ ...state, loading: true })); setReserve(state => ({ ...state, loading: true }));
  ...
  return <>
    ...
    <Panel title="Advertising spend" subtitle="By provider · selected period">{!marketing.rows.length ? unavailable(marketing) : spend.some(r => r.value === null) ? <MetricState label="Incomplete spend metrics" /> : distribution(spend, 'ad spend', formatCurrency)}</Panel>
