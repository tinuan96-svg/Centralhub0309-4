...
export default function SecurityPulse({ selectedStoreId = 'all', compact = false }: { selectedStoreId?: string; compact?: boolean }) {
  ...
  return <section className={`ch-panel ch-security-pulse ${compact ? 'ch-security-pulse-compact' : ''}`} aria-label="Live security pulse">
    ...
    <div className="ch-security-footer"><span><Activity size={13} /> {connection === 'live' ? 'Realtime events + 30s safety check' : connection === 'offline' ? 'Offline · last values retained' : '30s safety check active'}</span><Link href="/site-health">Open Security Centre →</Link></div>
  </section>;
}
