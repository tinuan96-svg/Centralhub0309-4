  ...
  return (
    <div className="p-4 md:p-6 space-y-8 pb-32 max-w-[1600px] mx-auto">
      ...
      <section id="overview" className="scroll-mt-24 space-y-4">
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-3"><StatCard label="Visitors Now" value={loading || errors.some(e => e.startsWith('Realtime:')) ? '—' : liveTotals.users.toLocaleString()} icon="👥" variant="glass" /><StatCard label="Sessions Now" value={loading || errors.some(e => e.startsWith('Realtime:')) ? '—' : liveTotals.sessions.toLocaleString()} icon="⚡" variant="glass" /><StatCard label="User counts (30d)" value={currentRows.length ? current.users.toLocaleString() : '—'} icon="📈" variant="glass" /><StatCard label="30d Orders" value={currentRows.length ? current.purchases.toLocaleString() : '—'} icon="🛒" variant="glass" /><StatCard label="30d Revenue" value={currentRows.length ? money(current.revenue) : '—'} icon="💎" variant="glass" /><StatCard label="Search Clicks" value={searchRows.length ? searchTotals.clicks.toLocaleString() : '—'} icon="🔎" variant="glass" /></div>
      </section>
      ...
      <section id="growth" className="scroll-mt-24 space-y-4">
        ...
        ].map(item => <Card key={item.label} className="p-4 bg-slate-900/40 border-slate-800 rounded-2xl"><p className="text-[10px] uppercase tracking-widest text-slate-500 font-bold">{item.label}</p><p className="text-2xl font-black text-white mt-1">{item.value}</p><p className={`text-xs mt-1 font-bold ${item.change === null ? 'text-slate-400' : item.change >= 0 ? 'text-emerald-400' : 'text-rose-400'}`}>{item.change === null ? 'No comparable baseline' : (item.change >= 0 ? '+' : '') + item.change.toFixed(1) + '% vs previous 30d'}</p></Card>)}</div>
        <TimeSeriesChart title="Website activity" subtitle="Daily imported session and purchase counts. Missing dates are not assumed to be zero." timeRange="30days" unit="count" series={[{ id: 'sessions', name: 'Sessions', color: '#67e8f9', data: trend.map(([date, row]) => ({ date, value: row.sessions })) }, { id: 'purchases', name: 'Purchases', color: '#a78bfa', data: trend.map(([date, row]) => ({ date, value: row.purchases })) }]} />
        <p className="ch-muted">User counts sum the imported daily source rows; they are not unique people across the full period.</p>
      ...
      <section id="ecommerce" className="scroll-mt-24 space-y-4"><Panel title="Shopping activity" subtitle="Recorded event counts; this is not a unique-user conversion funnel."><MetricBars data={currentRows.length ? [{ label: 'Product views', value: current.productViews }, { label: 'Add to cart', value: current.carts }, { label: 'Checkouts', value: current.checkouts }, { label: 'Purchases', value: current.purchases }] : []} /></Panel><div><h2 className="text-xl font-black text-white">Ecommerce & Conversion Analytics</h2><p className="text-sm text-slate-500">Website journey from product view through checkout and purchase.</p></div><div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-8 gap-3">{[
