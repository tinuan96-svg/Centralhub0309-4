  ...
  return (
    <tr onClick={onOpenDetail} className={`border-b border-slate-800/40 hover:bg-slate-800/30 transition-colors cursor-pointer ${isSelected ? 'bg-blue-900/10' : ''}`}>
      ...
      <td className="px-3 py-3">
        {store ? <StoreBadge store={store} size="sm" /> : <div className="flex items-center gap-1"><div className="w-2 h-2 rounded-full bg-slate-600" /><span className="text-[10px] text-slate-500 font-medium">No Store</span></div>}
      </td>
...
export default function OrdersPage({ params, searchParams }: { params: any; searchParams: any }) {
  ...
  if (isLoading && orders.length === 0) return <div className="p-4 sm:p-8 text-center text-slate-500 uppercase font-black tracking-widest animate-pulse text-sm sm:text-base">Loading Payment-Successful Orders...</div>;

  const paymentTabs: Array<{ id: PaymentStatus | 'all'; label: string }> = [
    { id: 'paid', label: '✅ Payment Successful' },
    ...
    <div className="p-3 sm:p-4 space-y-4 sm:space-y-6">
      <div className="sticky top-14 md:static z-20 bg-slate-950/80 backdrop-blur-md md:bg-transparent -mx-3 px-3 py-3 border-b md:border-0 border-slate-800/50 flex flex-col lg:flex-row lg:items-end justify-between gap-4 sm:gap-6">
         <div className="space-y-3 sm:space-y-4 min-w-0">
            <div className="flex items-center gap-2 sm:gap-4 flex-wrap">
              <h1 className="text-xl sm:text-2xl font-black text-slate-100 uppercase tracking-tighter">Payment-Successful Orders</h1>
            ...
            <p className="text-xs text-slate-500 max-w-3xl">Default view shows payment-received orders only. Pending, failed and refunded attempts remain available through the payment filter for audit/support, but they do not show profit as business performance.</p>
            ...
            <div className="ch-card flex gap-1 overflow-x-auto no-scrollbar bg-slate-900/50 p-1 rounded-2xl border border-slate-800/50">
               {paymentTabs.map(t => <button key={t.id} onClick={() => { setFilterPayment(t.id); setCurrentPage(1); }} className={`px-3 sm:px-5 py-2 sm:py-2.5 rounded-xl text-[10px] sm:text-xs font-black uppercase tracking-widest transition-all whitespace-nowrap ${filterPayment === t.id ? 'bg-emerald-700/70 text-white shadow-lg border border-emerald-500/40' : 'text-slate-500 hover:text-slate-300'}`}>{t.label}</button>)}
            </div>
            <div className="ch-card flex gap-1 overflow-x-auto no-scrollbar bg-slate-900/50 p-1 rounded-2xl border border-slate-800/50">
         ...
         <div className="flex items-center gap-2 sm:gap-3"><button onClick={() => setShowArchived(!showArchived)} className={`flex-1 md:flex-none px-3 sm:px-4 py-2 rounded-lg text-[10px] font-black uppercase tracking-widest transition-all ${showArchived ? 'bg-slate-800 text-white' : 'text-slate-500'}`}>{showArchived ? 'Hide Archived' : 'Show Archived'}</button></div>
      </div>
      ...
      <div className="md:hidden space-y-3">
        ...
        {orders.map(o => {
          ...
          return <div key={o.id} onClick={() => setDetailOrder(o)} className="ch-card bg-slate-900/50 border border-slate-800 rounded-2xl p-4 space-y-3 active:bg-slate-800/40 transition-colors cursor-pointer shadow-md">
      ...
      <div className="md:hidden fixed bottom-[64px] left-0 right-0 p-4 bg-slate-900/90 backdrop-blur-md border-t border-slate-800 z-30 flex gap-3 pb-safe-bottom"><div className="flex-1 flex gap-1 bg-slate-950 border border-slate-800 rounded-2xl p-1 overflow-x-auto no-scrollbar">{['all', 'today', '7days'].map(r => <button key={r} onClick={() => { setTimeRange(r as any); setCurrentPage(1); }} className={`px-4 py-2.5 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all ${timeRange === r ? 'bg-cyan-600 text-white shadow-lg' : 'text-slate-500'}`}>{r}</button>)}</div></div>
