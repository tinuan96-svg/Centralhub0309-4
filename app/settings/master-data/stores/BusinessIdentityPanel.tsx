...
type IdentityForm = {
  ...
  verification_status: 'not_verified' | 'pending' | 'verified' | 'needs_review';
  notes: string;
};
...
export default function BusinessIdentityPanel({ storeId, compact = false }: Props) {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      const { data, error } = await supabase
        ...
        .select('legal_company_name,trading_name,company_registration_number,vat_number,legal_address,city,postcode,country,business_email,business_phone,website_domain,verification_status,notes')
        .eq('store_id', storeId)
        .maybeSingle();
      ...
      setIdentity(data ? { ...EMPTY, ...data, notes: data.notes || '' } as IdentityForm : { ...EMPTY });
      setLoading(false);
    })();
    return () => { active = false; };
  }, [storeId]);

  const update = (key: keyof IdentityForm, value: string) => {
  ...
  const fields: Array<[keyof IdentityForm, string]> = [
  ...
  return <section className={`rounded-2xl border border-slate-800 bg-slate-900/40 ${compact ? 'p-4' : 'p-6'}`}>
    <div className="flex items-start justify-between gap-4 mb-5">
      <div>
        <h3 className="text-base font-black text-white">Business Identity</h3>
        <p className="text-xs text-slate-500 mt-1">This identifies only the selected store/company for its own external platform connections.</p>
      </div>
    ...
    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
      {fields.map(([key, label]) => <label key={key} className="block">
