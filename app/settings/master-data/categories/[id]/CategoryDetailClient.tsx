...
export default function CategoryDetailClient({ params: _params, searchParams: _searchParams }: { params: any; searchParams: any }) {
  ...
  const loadCategoryAnalytics = async () => {
    ...
    const [detail, trend] = await Promise.all([
      IntelligenceService.getCategoryDetail(id as string, options),
      IntelligenceService.getTrendData(options, id as string, 'category')
    ]);
  ...
  const brandChartData = useMemo(() => {
    if (!data?.brandContribution) return [];
    return data.brandContribution.map((bc: any) => ({
      label: bc.brandName,
      value: bc.revenue,
      secondaryValue: bc.units
  ...
  const insights = useMemo(() => {
    ...
    if (m.revenue > 1000) {
      res.push({
        type: 'success',
        title: 'High Revenue Category',
        text: `This category is a major revenue driver, generating ${fmt(m.revenue)} in the selected period.`
      });
    }
    ...
    if (m.margin > 30) {
      res.push({
        type: 'success',
        title: 'High Margin',
        text: `Exceptional profitability with an average margin of ${m.margin.toFixed(1)}%.`
      });
    } else if (m.margin < 15 && m.revenue > 0) {
      res.push({
        type: 'warning',
        title: 'Low Margin Alert',
        text: `Profitability is below targets at ${m.margin.toFixed(1)}%. Investigate cost prices.`
      });
    }

    if (m.activeProducts < m.totalProducts * 0.5) {
      res.push({
        type: 'info',
        title: 'Catalog Optimization',
        text: `Only ${(m.activeProducts / m.totalProducts * 100).toFixed(0)}% of products in this category are active.`
      });
    }
  ...
  if (loading) return <div className="p-8 text-center text-slate-500 uppercase font-black tracking-widest animate-pulse">Analyzing Category Performance...</div>;
  if (!data) return <div className="p-8 text-center text-rose-400 font-bold">Category not found or error loading data.</div>;
    ...
    <div className="space-y-8">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
        <div className="flex items-center gap-6">
          ...
          <div className="w-20 h-20 rounded-3xl bg-emerald-500/10 flex items-center justify-center text-4xl shadow-2xl border border-emerald-500/20 text-emerald-400 font-black">
            {data.category.name[0]?.toUpperCase()}
          </div>
          <div>
            <h1 className="text-3xl md:text-5xl font-black text-white uppercase tracking-tighter">{data.category.name}</h1>
            <div className="flex items-center gap-3 mt-2">
              <span className={`px-2.5 py-0.5 rounded-full text-[10px] font-black uppercase border ${data.category.is_active ? 'bg-emerald-500/10 text-emerald-400 border-emerald-500/20' : 'bg-rose-500/10 text-rose-400 border-rose-500/20'}`}>
                {data.category.is_active ? 'Active' : 'Inactive'}
              </span>
              <span className="text-slate-500 text-xs font-bold uppercase tracking-widest">Category Intelligence</span>
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="space-y-6">
           <h3 className="text-sm font-black text-slate-400 uppercase tracking-widest px-2">Category Insights</h3>
      ...
      <div className="grid grid-cols-1 gap-8">
        <CategoryBarChart data={brandChartData} title="Brand Contribution (Market Share)" />
      </div>

      {/* Product Performance Table */}
      <div className="bg-slate-900/50 rounded-[2rem] border border-slate-800 overflow-hidden shadow-2xl">
        <div className="p-6 border-b border-slate-800 flex items-center justify-between">
          <h3 className="text-sm font-black text-slate-400 uppercase tracking-widest">Top Products in Category</h3>
        </div>
            ...
            <tbody className="divide-y divide-slate-800/50">
              {data.products.map((p: any) => (
                <tr key={p.id} className="hover:bg-slate-800/30 transition-colors">
                  ...
                  <td className="px-6 py-4">
                    <span className="text-xs font-bold text-slate-400 uppercase">{p.brandName}</span>
                  </td>
