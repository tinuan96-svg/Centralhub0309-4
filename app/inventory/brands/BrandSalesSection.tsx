...
export default function BrandSalesSection() {
  ...
  return <div className="space-y-5">
    <div className="flex flex-wrap items-center gap-2 rounded-2xl border border-slate-800 bg-slate-900/40 p-2">
      ...
      <button type="button" onClick={() => setTab('manage')} className={`rounded-xl px-4 py-2.5 text-[10px] font-black uppercase tracking-widest ${tab === 'manage' ? 'bg-blue-600 text-white' : 'text-slate-500 hover:text-slate-200'}`}>⚙️ Manage brands</button>
      <span className="ml-auto hidden text-[9px] font-black uppercase tracking-widest text-slate-600 md:block">Same visual system as category intelligence</span>
    </div>
    {tab === 'graphs' ? <SalesGraphDashboard focus="brand" /> : <BrandsClient params={{}} searchParams={{}} />}
  </div>;
}
