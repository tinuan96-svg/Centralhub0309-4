...
export default function ExpiryClient({ params, searchParams }: { params: any; searchParams: any }) {
  ...
  const loadData = async () => {
    setLoading(true);
    const { data: products } = await supabase.from('products').select('id, name, sku, stock, expiry_date, image_url').not('expiry_date', 'is', null).order('expiry_date', { ascending: true });
    setData(products || []);
    setLoading(false);
  };
  ...
  return (
    <div className="p-6 max-w-[1200px] mx-auto space-y-6">
      <div className="flex items-center justify-between">
        <div><h1 className="text-xl font-bold text-slate-100">Grocery Expiry Management</h1><p className="text-sm text-slate-500 mt-1">Track products nearing expiration to minimize wastage</p></div>
