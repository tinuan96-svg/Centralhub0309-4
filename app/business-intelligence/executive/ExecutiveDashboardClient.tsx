...
export default function ExecutiveDashboardClient({ params, searchParams }: { params: any; searchParams: any }) {
  ...
  const loadData = useCallback(async () => {
    setLoading(true);
    try {
      const [profit, featureFlags, quality, groups, custIntel] = await Promise.all([
        ...
        supabase.from('intelligence_recommendation_groups').select('*').order('priority_score', { ascending: false }).limit(5),
        supabase.from('customer_intelligence').select('lifetime_value')
      ]);

      setProfitData(profit);
      setFlags(featureFlags);
      setQualityIssues(quality);
      setPriorities(groups.data || []);

      if (custIntel.data && custIntel.data.length > 0) {
         const totalLtv = custIntel.data.reduce((sum, c) => sum + (c.lifetime_value || 0), 0);
         setCustomerStats({
            avgLtv: totalLtv / custIntel.data.length,
            count: custIntel.data.length
         });
      }
    } catch (err) {
  ...
  const handleRefreshAll = async () => {
    setRefreshing(true);
    try {
       // Run background refreshes for all intelligence layers
       await Promise.all([
          intelligenceService.syncAllCustomerIntelligence(),
          productEconomicsService.refreshCache(50),
          inventoryForecastService.refreshAllForecasts(50)
       ]);
       await loadData();
    } catch (err) {
  ...
  const stats = useMemo(() => {
    // Consistency: Product-only revenue (Total - Delivery)
    const revenue = profitData.reduce((sum, o) => sum + o.total - o.delivery_fee, 0);
    const profit = profitData.reduce((sum, o) => sum + o.gross_profit, 0);
  ...
  const topStore = useMemo(() => {
    ...
    let totalRevenue = 0;
    profitData.forEach(o => {
      const rev = o.total - o.delivery_fee;
      const sId = o.store_id || 'unknown';
      storeRevenue[sId] = (storeRevenue[sId] || 0) + rev;
      totalRevenue += rev;
    });

    let maxRev = -1;
    let maxId = '';
    Object.entries(storeRevenue).forEach(([id, rev]) => {
      if (rev > maxRev) {
        maxRev = rev;
        maxId = id;
      }
    ...
    const storeName = profitData.find(o => (o.store_id || 'unknown') === maxId)?.store_id || 'Unknown';

    return {
      name: storeName,
      share: totalRevenue > 0 ? (maxRev / totalRevenue) * 100 : 0
    };
  }, [profitData]);
  ...
  return (
    <div className="p-6 space-y-8">
        ...
        action={
          <div className="flex gap-2">
             <Button onClick={handleRefreshAll} disabled={refreshing}>
                {refreshing ? 'Analyzing...' : 'Run Full Intelligence Scan'}
             </Button>
      ...
      <StatGrid columns={4}>
        ...
        <StatCard label="Gross Profit" value={formatCurrency(stats.profit)} icon="📈" trend={{ value: 5.2, isPositive: true }} />
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div id="approvals" className="lg:col-span-2">
          <Card className="p-6 h-full bg-slate-900/50 border-slate-800">
            ...
            <div className="space-y-4">
               {priorities.map(p => (
                 <div key={p.id} className="p-4 rounded-2xl bg-slate-800/40 border border-slate-700/50 flex justify-between items-center group hover:border-blue-500/30 transition-all">
                    <div className="flex items-center gap-4">
                       <div className="w-10 h-10 rounded-full bg-slate-950 flex items-center justify-center text-xl">
                          {p.primary_impact_type === 'profit' ? '💰' : p.primary_impact_type === 'inventory_safety' ? '📦' : '📈'}
                       </div>
                       <div>
                    ...
                    <div className="text-right">
                       <Badge variant={p.priority_score > 80 ? 'danger' : 'warning'} className="text-[9px]">PRIORITY {p.priority_score}</Badge>
                       <p className="text-[10px] text-slate-500 mt-1 uppercase font-black">{p.confidence_level} Confidence</p>
                    </div>
                 </div>
               ))}
               {priorities.length === 0 && (
                 <div className="py-20 text-center border border-dashed border-slate-800 rounded-2xl">
                    <p className="text-slate-500 italic">No coordinated priorities identified. The system is calculating...</p>
                 </div>
               )}
            </div>
          </Card>
        ...
        {/* Coordinated Intelligence Brief */}
          ...
          <div className="space-y-4 flex-1">
                  ...
                  <ul className="space-y-1">
                     {qualityIssues.map((q, i) => (
                       <li key={i} className="text-[11px] text-slate-300 flex items-start gap-2">
                          <span className={q.severity === 'critical' ? 'text-rose-500' : 'text-amber-500'}>•</span>
                          {q.issue}
                       </li>
                     ))}
                  </ul>
            ...
            <div className="p-4 rounded-xl bg-blue-500/5 border border-blue-500/10">
              ...
              <p className="text-sm text-slate-300 leading-relaxed">
                 {priorities.length > 0
                   ? `High-priority action identified: ${priorities[0].title}. Recommended next step: review impact analysis.`
                   : "No critical market insights requiring immediate attention."}
              </p>
            </div>
            ...
            <div className="p-4 rounded-xl bg-emerald-500/5 border border-emerald-500/10">
              ...
              <p className="text-sm text-slate-300">
                 All intelligence modules are operational. {qualityIssues.length === 0 ? "Data quality is within acceptable parameters." : "Review quality alerts for potential data gaps."}
              </p>
            </div>
          </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
         <Card className="p-4 border-slate-800">
            ...
            <div className="flex justify-between items-end">
               <span className="text-lg font-bold text-white truncate max-w-[120px]">{topStore.name}</span>
               <Badge variant="success">{topStore.share.toFixed(0)}% Share</Badge>
            </div>
         </Card>
         <Card className="p-4 border-slate-800">
            <p className="text-[10px] text-slate-500 font-black uppercase mb-3">Customer LTV</p>
            <div className="flex justify-between items-end">
               <span className="text-lg font-bold text-white">{customerStats.count > 0 ? formatCurrency(customerStats.avgLtv) : '—'}</span>
               <span className="text-[10px] text-slate-400">{customerStats.count} customers</span>
            </div>
         </Card>
         <Card className="p-4 border-slate-800">
            <p className="text-[10px] text-slate-500 font-black uppercase mb-3">CAC</p>
            <div className="flex justify-between items-end">
               <span className="text-lg font-bold text-slate-500 italic text-sm">DATA N/A</span>
               <span className="text-[10px] text-slate-500">No ad-spend data</span>
            </div>
         </Card>
         <Card className="p-4 border-slate-800">
            ...
            <div className="flex justify-between items-end">
               <span className="text-lg font-bold text-slate-500 italic text-sm">DATA N/A</span>
               <span className="text-[10px] text-slate-500">Awaiting attribution</span>
            </div>
         </Card>
      </div>
    </div>
