...
## Product propagation
The private `tasty_kerala_operational_snapshot()` builds an allowlisted set of all CentralHub SKU records, stock (expiry-adjusted), price, cost, operational statuses, category/image/description/SEO and variant links. `tasty_kerala_push_operational_snapshot()` sends it to the Tasty-local Edge receiver through pg_net every five minutes using the CentralHub Vault token. Tasty independently authenticates the caller against CentralHub's boolean-only token verification RPC; no CentralHub service role key or secret is committed into a repository.
