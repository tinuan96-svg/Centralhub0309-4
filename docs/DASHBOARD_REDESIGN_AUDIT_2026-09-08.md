...
## Denser first view and automatic dashboard updates

This refinement starts from `c548eba94fa22b378d37a099adc9742ecb561af4`. The supplied long screenshot showed large unused card areas and the reference composition falling back to two columns. The main reference grid now switches to its wide composition at 900px. Smaller plots and content-height cards reduce empty space. Secondary metric cards use measured grid spans to fill gaps instead of stretching a short card to the height of its tallest neighbour. The existing global mobile/sidebar breakpoint is unchanged.

The first view adds a compact operational strip: all orders, eligible paid orders, undelivered paid orders, average paid product sale, current shared low-stock count and incomplete-cost count. Sales and gross-profit totals include small charts from the same daily series. Repeat-buyer details include single-order buyers and orders per identified buyer for the selected period. These values are derived from the existing report, with the same exclusions and unknown-value handling. The new density option on the shared time-series chart is opt-in; its default geometry and exact-value table remain intact.

Read-only inspection of `pg_publication_tables` confirmed that orders, products, inventory movements, WhatsApp contacts/conversations/messages, financial plan projects/milestones, system notifications and the new security tables already support database events. The dashboard uses one authenticated public-schema subscription with an explicit dashboard-table allowlist and the documented [Supabase Postgres Changes API](https://supabase.com/docs/guides/realtime/postgres-changes). This receives authorized published records without requesting invalid bindings for unpublished report sources. No publications, RLS policies, credentials or synchronization settings were changed by this refinement.

Changes are grouped over a 750ms window and report reads are serialized. A change received during a read schedules one follow-up read. Other source summaries are re-read after successful report loads, with a 30-second foreground check covering tables/views that do not publish events. Reconnection and returning to the page request an update. The loop pauses while the page is hidden or offline and is removed when leaving the overview. This refreshes dashboard reads; it does not initiate external bank, advertising or analytics imports. Imported provider data retains the provider's reporting delay.

The status strip displays the connection/update state, age of the last successful core report load and a clock. Its rotating indicator represents the data feed and refresh activity. It does not claim security scanning. Errors and delayed report updates remain visible; the last successful core snapshot is explicitly marked when refresh fails. Filter changes cannot display a snapshot from a previous scope. Background reads preserve mounted charts and their selected inspection points instead of replacing the entire console with loading placeholders. Value highlights occur when the displayed value changes, without interpolating fictional intermediate amounts; reduced-motion preferences disable these animations.

During release preparation, security-monitor work landed on main at `b7d1324f27fb1173cb97be0f31549eab79d70c39`. Its Security Pulse, Site Health update, security edge function, migration and added continuity assertions were retained. The primary radar has compact dashboard styling. The new core-report read queue consumes the upstream `useDashboardRealtime` hook, enhanced with one 30-second foreground timer, event filtering and lifecycle cleanup; a second competing core-report subscription was removed. Security Pulse continues to own its independent telemetry display. The security backend and scheduled monitoring are unchanged from that upstream revision.

Validation passed on the combined revision: lint, 128-page route integrity, 93 critical pages, 110 navigation targets, 71 feature files and all 50 behavior groups, runtime and order-sync contracts, TypeScript, production build, 116 static-route smoke checks, the reporting audit and whitespace check. Added tests cover burst coalescing, no overlapping report reads, updates arriving during a fetch, relevant versus unrelated events, the actual transport's hidden/offline/resume/error behavior, timer/channel/listener disposal, packed-card span bounds and empty/zero/signed small trends. The local compilation uses the same existing missing-environment fallback; no authenticated browser screenshot comparison or device visual test was performed. Production CI and deployment are verified separately on the released revision.
